...
export default function KanbanCard({ event, column, onDelete }) {
  ...
  // Comentários (com usuário logado + otimista)
  const handleAddComment = async () => {
    if (!newCommentText.trim()) return;
    if (!currentUser || !currentUser.id) {
      alert("Selecione um usuário na Home para comentar.");
      return;
    }
    ...
    try {
      ...
      // Troca o otimista pelo salvo do backend
  ...
  const renderAuthorName = (comment) =>
    (comment?.author && (comment.author.name || comment.author.fullName)) ||
    comment?.authorName ||
    (typeof comment?.author === "string" ? comment.author : "") ||
    "Usuário";

  const renderAuthorInitial = (comment) => {
    const name = renderAuthorName(comment);
